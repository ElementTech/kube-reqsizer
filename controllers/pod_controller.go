								...
								if pod.Spec.Containers[i].Resources.Requests != nil {
									...
									case "average":
										if r.ValidateCPU(currentC.CPU, AverageUsageCPU) {
											pod.Spec.Containers[i].Resources.Requests[v1.ResourceCPU] = resource.MustParse(fmt.Sprintf("%dm", AverageUsageCPU*r.CPUFactor))
											PodChange = true
										}
									case "min":
										if r.ValidateCPU(currentC.CPU, c.MinCPU) {
											pod.Spec.Containers[i].Resources.Requests[v1.ResourceCPU] = resource.MustParse(fmt.Sprintf("%dm", c.MinCPU*r.CPUFactor))
											PodChange = true
										}
									case "max":
										if r.ValidateCPU(currentC.CPU, c.MaxCPU) {
											pod.Spec.Containers[i].Resources.Requests[v1.ResourceCPU] = resource.MustParse(fmt.Sprintf("%dm", c.MaxCPU*r.CPUFactor))
											PodChange = true
										}
									}
								}
									...
									if pod.Spec.Containers[i].Resources.Requests != nil {
										...
										case "average":
											if r.ValidateMemory(currentC.Memory, AverageUsageMemory) {
												pod.Spec.Containers[i].Resources.Requests[v1.ResourceCPU] = resource.MustParse(fmt.Sprintf("%dm", AverageUsageMemory*r.MemoryFactor))
												PodChange = true
											}
										case "min":
											if r.ValidateMemory(currentC.Memory, c.MinMemory) {
												pod.Spec.Containers[i].Resources.Requests[v1.ResourceCPU] = resource.MustParse(fmt.Sprintf("%dm", c.MinMemory*r.MemoryFactor))
												PodChange = true
											}
										case "max":
											if r.ValidateMemory(currentC.Memory, c.MaxMemory) {
												pod.Spec.Containers[i].Resources.Requests[v1.ResourceCPU] = resource.MustParse(fmt.Sprintf("%dm", c.MaxMemory*r.MemoryFactor))
												PodChange = true
											}
										}
									}
